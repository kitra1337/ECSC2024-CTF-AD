...
app.get("/question/:qid", async function (req, res) {
    ...
    if (["1", "2", "3", "4", "5"].indexOf(qid) === -1) {
        existingQuestion = await questions.findOne(new ObjectId(qid));
    } else {
        existingQuestion = await questions.findOne({ id: +qid });
        existingQuestion._id = existingQuestion.id.toString();
    }
...
app.get("/qimages/:qid", async function (req, res) {
    ...
    if (qid) {
        ...
        if (["1", "2", "3", "4", "5"].indexOf(qid) === -1) {
            imagename = (await questions.findOne(new ObjectId(qid))).image;
        } else {
            imagename = (await questions.findOne({ id: +qid })).image;
        }
...
app.post("/createchallenge", async function (req, res) {
    ...
    const doc = await questions.insertOne(challenge);

    return res.json({ message: "Ok", id: doc.insertedId });
});
