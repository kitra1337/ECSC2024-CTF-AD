...
class Client:
    ...
    async def fill_db(self):
        ...
        for _ in range(100):
            public = bool(rng.randint(0,1))
            X = 0
            if not public:
                x = rng.randrange(q)
                X = pow(g, x, p)
                
            playlist_id = await self.add_playlist(random_string(10), random_string(20), public, rng.sample(song_ids, k=rng.randint(0, len(songs))), X)
